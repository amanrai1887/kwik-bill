...
export const InvoiceRenderer: React.FC<InvoiceRendererProps> = ({
  ...
  if (template === 'modern') {
    return (
      <div className="space-y-8 bg-white text-slate-800 font-sans p-6 sm:p-8">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs">
          <div>
            ...
            {invoice.client?.gstin && <p className="font-mono text-slate-600">GSTIN: {invoice.client.gstin}</p>}
          </div>

          {invoice.industryDetails?.vehicleNo ? (
            <div className="space-y-1">
              <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px] block mb-1">
                Transport Freight Manifest
              </span>
              ...
              <p><span className="text-slate-500">LR / Bilty No:</span> <strong className="font-mono text-slate-900">{invoice.industryDetails.lrNumber || 'N/A'}</strong></p>
              <p><span className="text-slate-500">Route:</span> <span className="text-slate-800 font-medium">{invoice.industryDetails.routeFrom || 'Origin'} → {invoice.industryDetails.routeTo || 'Destination'}</span></p>
            </div>
          ) : (
            <div className="space-y-1">
              <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px] block mb-1">
              ...
              <p>Bank: <strong>{profile?.bankName || 'HDFC Bank'}</strong></p>
              <p>A/C: <strong className="font-mono">{profile?.bankAccountNo || '50200084729103'}</strong></p>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold uppercase text-[10px]">
              <tr>
                ...
                <th className="py-2.5 px-4 w-6/12">Item / Service Description</th>
        ...
        <div className="border-t border-slate-200 pt-4 text-[11px] text-slate-500 space-y-1">
          ...
          {customFooter ? (
            <p className="text-slate-600 font-medium pt-1">{customFooter}</p>
          ) : (
            <p><strong>Terms:</strong> {invoice.terms || '2% late fee per month on overdue invoices.'}</p>
          )}
        </div>
      </div>
    );
      ...
      <div className="space-y-6 bg-white text-slate-900 font-serif p-6 sm:p-10 border-t-8" style={{ borderTopColor: brandColor }}>
        ...
        <div className="grid grid-cols-2 gap-6 p-4 bg-slate-100/70 border border-slate-300 font-sans text-xs">
          <div>
            ...
            <p className="font-mono text-slate-700 mt-1">GSTIN: {invoice.client?.gstin || 'Unregistered'}</p>
          </div>
          ...
          <tbody className="divide-y divide-slate-300">
            {invoice.items?.map((item, idx) => (
              <tr key={idx} className={idx % 2 === 1 ? 'bg-slate-50' : 'bg-white'}>
                ...
                <td className="p-2.5 border-r border-slate-300 text-center font-mono">{item.hsnCode || '-'}</td>
        ...
        <div className="flex justify-between items-end pt-4 font-sans text-xs">
          ...
          <div className="w-72 bg-slate-900 text-white p-4 rounded-lg space-y-2 font-mono text-xs">
      ...
      <div className="space-y-6 bg-white text-slate-900 font-sans p-6 sm:p-8 border-2 border-emerald-500 rounded-3xl">
        ...
        <div className="bg-emerald-50/70 border border-emerald-200 p-4 rounded-2xl grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          ...
          <div>
            ...
            <span className="font-semibold text-slate-800">{invoice.industryDetails?.routeFrom || 'JNPT Navi Mumbai'}</span>
          </div>
          <div>
            <span className="text-[10px] font-bold text-emerald-800 uppercase block">TO (DESTINATION)</span>
            <span className="font-semibold text-slate-800">{invoice.industryDetails?.routeTo || 'Sanand GIDC, Ahmedabad'}</span>
          </div>
        </div>
        ...
        <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-xs flex justify-between items-center">
          ...
          <div className="text-right">
            ...
            <p className="font-mono text-slate-700">+91 {invoice.client?.phone}</p>
          </div>
        </div>
    ...
    return (
      <div className="space-y-4 bg-white text-slate-900 font-sans p-6 border border-slate-300 text-xs">
        <div className="text-center pb-3 border-b-2 border-slate-900">
          ...
          <p className="text-[11px] font-mono">GSTIN: {profile?.gstin || '27AABCU9603R1ZN'} | State Code: 27 (Maharashtra)</p>
        </div>
        ...
        <table className="w-full border-collapse border border-slate-400 text-xs">
          <thead>
            <tr className="bg-slate-100 text-center font-bold">
              ...
              <th className="border border-slate-400 p-2 w-16">HSN</th>
              ...
              <th className="border border-slate-400 p-2 w-16">CGST</th>
              <th className="border border-slate-400 p-2 w-16">SGST</th>
          ...
          <tbody>
            {invoice.items?.map((item, idx) => {
              ...
              const itemCgst = (itemTaxable * (Number(invoice.taxRate) / 2)) / 100;
              ...
              return (
                <tr key={idx} className="text-center font-mono">
                  ...
                  <td className="border border-slate-400 p-2 text-right">₹{itemCgst.toFixed(2)}</td>
                  <td className="border border-slate-400 p-2 text-right">₹{itemSgst.toFixed(2)}</td>
        ...
        <div className="flex justify-between pt-2">
          <div className="text-[11px] text-slate-600 max-w-sm">
            ...
            <p className="mt-6">Authorized Signatory For <strong>{businessName}</strong></p>
          </div>
          <div className="w-64 space-y-1 font-mono text-xs text-right">
            <p>Total Taxable Amount: <strong>₹{subtotal.toFixed(2)}</strong></p>
            <p>CGST ({(Number(invoice.taxRate) / 2).toFixed(1)}%): <strong>₹{cgstAmount.toFixed(2)}</strong></p>
            <p>SGST ({(Number(invoice.taxRate) / 2).toFixed(1)}%): <strong>₹{sgstAmount.toFixed(2)}</strong></p>
  ...
  return (
    <div className="space-y-6 bg-slate-950 text-slate-100 font-sans p-6 sm:p-8 rounded-3xl border border-slate-800 shadow-2xl">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-400 uppercase tracking-widest bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20 mb-1">
              <Zap className="w-3 h-3 fill-current" /> Verified Tax Invoice
            </div>
      ...
      {/* UPI QR pill */}
      <div className="flex justify-between items-center p-4 bg-slate-900 border border-slate-800 rounded-2xl text-xs font-mono">
        <div>
